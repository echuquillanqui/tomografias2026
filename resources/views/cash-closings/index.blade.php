    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="card clinic-card">
                ...
                <div class="card-body p-0"><div class="table-responsive"><table class="table table-clinic align-middle mb-0"><thead><tr><th>Fecha</th><th>Orden</th><th>Paciente</th><th>Tipo de prueba</th><th>Contraste</th><th>Convenio</th><th>Pago</th><th>Tipo de comprobante</th><th>N° comprobante</th><th>Total</th></tr></thead><tbody>@forelse($orders as $order)<tr><td>{{ $order->fecha_orden->format('d/m/Y H:i') }}</td><td><a href="{{ route('orders.show', $order) }}" class="fw-bold">{{ $order->codigo_orden ?? '#'.$order->id }}</a></td><td>{{ $order->patient->nombres }} {{ $order->patient->apellidos }}</td><td>@forelse($order->orderExams as $orderExam)<div class="fw-semibold">{{ $orderExam->exam->nombre_examen ?? '—' }}</div>@empty — @endforelse</td><td>@forelse($order->orderExams as $orderExam)<div class="text-nowrap">{{ $orderExam->tipo_contraste ?? '—' }}</div>@empty — @endforelse</td><td>{{ $order->agreement->nombre_institucion }}</td><td>{{ $order->tipo_pago ?? '—' }}</td><td>{{ $order->tipo_comprobante ?? '—' }}</td><td>{{ $order->numero_comprobante ?? '—' }}</td><td class="text-success fw-bold">S/ {{ number_format($order->total, 2) }}</td></tr>@empty<tr><td colspan="10" class="text-center py-4">Sin órdenes en el rango.</td></tr>@endforelse</tbody></table></div></div>
            </div>
        </div>
        </div>
        </div>
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered align-middle mb-0 small">
                            ...
                            <tfoot><tr class="table-warning fw-bold"><td colspan="9" class="text-end">TOTAL INGRESOS</td><td class="text-center">{{ number_format($operationalIopamidolSummary['delivered'], 2) }}</td><td colspan="2"></td><td class="text-end">{{ number_format($operationalIncomeTotal, 2) }}</td><td class="text-end">{{ number_format($operationalYapePlinIncome, 2) }}</td><td class="text-end">{{ number_format($operationalTransferIncome, 2) }}</td><td></td><td class="text-center">{{ number_format($operationalPlateSummary['delivered']) }}</td><td class="text-center">{{ number_format($operationalPlateSummary['final']) }}</td><td class="text-center">{{ number_format($operationalIopamidolSummary['final'], 2) }}</td><td colspan="2" class="text-end">TOTAL GASTOS DÍA</td><td colspan="2" class="text-end text-danger">S/ {{ number_format($operationalExpenseTotal, 2) }}</td></tr></tfoot>
                        </table>
                    </div>
                </div>
            </div>
