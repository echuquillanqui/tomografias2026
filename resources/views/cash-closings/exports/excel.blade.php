...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
    ...
    <Worksheet ss:Name="Resumen">
        <Table>
            ...
            <Row><Cell><Data ss:Type="String">Iopamidol utilizado</Data></Cell><Cell><Data ss:Type="Number">{{ $iopamidolSummary['delivered'] }}</Data></Cell></Row>
            <Row><Cell><Data ss:Type="String">Iopamidol final</Data></Cell><Cell><Data ss:Type="Number">{{ $iopamidolSummary['final'] }}</Data></Cell></Row>
        </Table>
    </Worksheet>
    ...
    <Worksheet ss:Name="Cuadre diario">
        <Table>
            <Column ss:Width="35"/><Column ss:Width="70"/><Column ss:Width="150"/><Column ss:Width="75"/><Column ss:Width="100"/><Column ss:Width="110"/><Column ss:Width="110"/><Column ss:Width="170"/><Column ss:Width="65"/><Column ss:Width="125"/><Column ss:Width="75" ss:Span="5"/><Column ss:Width="130"/><Column ss:Width="140"/><Column ss:Width="130"/><Column ss:Width="85"/>
            <Row ss:Height="30"><Cell ss:StyleID="Title" ss:MergeAcross="21"><Data ss:Type="String">DETALLE DE ATENCIONES</Data></Cell></Row>
            <Row ss:Height="32"><Cell ss:StyleID="Header"><Data ss:Type="String">N°</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Fecha</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Paciente</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">DNI</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Orden de servicio</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Tipo de comprobante</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">N° comprobante</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Tipo de tomografía</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">S/C C/C</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Convenio / Institución</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Efectivo</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Yape</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Transferencia</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Por cobrar</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Placas usadas</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Saldo placas</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Iopamidol usado</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Saldo Iopamidol</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Médico solicitante</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Doctor informe</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Gasto</Data></Cell><Cell ss:StyleID="Header"><Data ss:Type="String">Monto gasto</Data></Cell></Row>
            @php $plateRunning = $plateSummary['initial']; $iopamidolRunning = $iopamidolSummary['initial']; $maxRows = max($orders->count(), $expenses->count()); @endphp
            @for($i = 0; $i < $maxRows; $i++)
                @php $order = $orders->values()->get($i); $expense = $expenses->values()->get($i); $plates = $order ? (float) (($order->admissionForm?->data['delivery_quantities']['PLACAS'] ?? $order->admissionForm?->data['plates_count'] ?? 0)) : 0; $iopamidol = $order ? (float) $order->consumables->filter(fn ($item) => str_contains(strtolower($item->reagent->nombre ?? ''), 'iopamidol'))->sum('cantidad') : 0; $plateRunning -= $plates; $iopamidolRunning -= $iopamidol; @endphp
                <Row><Cell ss:StyleID="Integer"><Data ss:Type="Number">{{ $i + 1 }}</Data></Cell><Cell ss:StyleID="Centered"><Data ss:Type="String">{{ $order?->fecha_orden?->format('d/m/Y') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order ? trim(($order->patient->nombres ?? '').' '.($order->patient->apellidos ?? '')) : '') }}</Data></Cell><Cell ss:StyleID="Centered"><Data ss:Type="String">{{ $text($order->patient->dni ?? '') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order?->codigo_orden ?? '') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order?->tipo_comprobante ?? '') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order?->numero_comprobante ?? '') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order?->orderExams?->pluck('exam.nombre_examen')->filter()->join(' + ') ?? '') }}</Data></Cell><Cell ss:StyleID="Centered"><Data ss:Type="String">{{ $text($order?->orderExams?->pluck('tipo_contraste')->filter()->join(', ') ?? '') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order?->agreement?->nombre_institucion ?? '') }}</Data></Cell><Cell ss:StyleID="Money"><Data ss:Type="Number">{{ $order ? $money($order->paymentAmount('Efectivo')) : 0 }}</Data></Cell><Cell ss:StyleID="Money"><Data ss:Type="Number">{{ $order ? $money($order->paymentAmount('Yape/Plin')) : 0 }}</Data></Cell><Cell ss:StyleID="Money"><Data ss:Type="Number">{{ $order ? $money($order->paymentAmount('Transferencia')) : 0 }}</Data></Cell><Cell ss:StyleID="Money"><Data ss:Type="Number">{{ $order ? $money($order->paymentAmount('Convenio')) : 0 }}</Data></Cell><Cell ss:StyleID="Integer"><Data ss:Type="Number">{{ $plates }}</Data></Cell><Cell ss:StyleID="Integer"><Data ss:Type="Number">{{ $order ? $plateRunning : 0 }}</Data></Cell><Cell ss:StyleID="Integer"><Data ss:Type="Number">{{ $iopamidol }}</Data></Cell><Cell ss:StyleID="Integer"><Data ss:Type="Number">{{ $order ? $iopamidolRunning : 0 }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order->medicoSolicitante->nombre ?? '') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($order->medicoInforme->nombre_completo ?? 'SIN INFORME') }}</Data></Cell><Cell><Data ss:Type="String">{{ $text($expense->descripcion ?? '') }}</Data></Cell><Cell ss:StyleID="Money"><Data ss:Type="Number">{{ $expense ? $money($expense->monto) : 0 }}</Data></Cell></Row>
            @endfor
        </Table>
    </Worksheet>
